# Deploy Max Clean v11 — TNreview (DQA TaskHub)
...
**v11:** Tách **Kết luận** / **Ghi chú** / **Nhận xét trưởng nhóm** trên Duyệt nhanh; map `Điểm thay đổi BC`; nút mở Dashboard SPM; session `credentials: same-origin`.
...
1. **Upload** `deploy-max-clean-v11.zip` lên server, giải nén vào thư mục app (vd. `~/tnreview`).
...
2. Upload & **giải nén** `deploy-max-clean-v11.zip` **vào** `public_html/pheduyet/` (phải thấy `server.js`, `index.html`, `form5-dist`, `.htaccess`).
...
2. Upload **`app.js` mới** từ zip v11 (chỉ có `import './server.js';`) **hoặc** đổi Startup file thành **`server.js`**.
3. **Restart** Node app.
...
**Sửa:** Upload **`app.cjs`** từ zip v11 vào `public_html/pheduyet/`, hoặc trong Node.js App đặt **Application startup file** = `app.cjs` (file phải tồn tại).
...
| **Trang trắng `/`** | Thiếu `index.html` cùng cấp `server.js` | Không deploy chỉ `dist/` — dùng layout gói v11 |
...
./scripts/build-deploy-max-clean-v11.sh
```

Hoặc thủ công: copy các file runtime vào `deploy-max-clean-v11/` rồi `zip -r deploy-max-clean-v11.zip deploy-max-clean-v11`.
