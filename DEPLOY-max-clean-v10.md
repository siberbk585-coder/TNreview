# Deploy Max Clean v10 — TNreview (DQA TaskHub)
...
**v10:** Cập nhật `index.html` mới nhất; login trống; UI báo cáo; `app.cjs`/`app.js`/htaccess cho cPanel.
...
1. **Upload** `deploy-max-clean-v10.zip` lên server, giải nén vào thư mục app (vd. `~/tnreview`).
...
2. Upload & **giải nén** `deploy-max-clean-v10.zip` **vào** `public_html/pheduyet/` (phải thấy `server.js`, `index.html`, `form5-dist`, `.htaccess`).
...
2. Upload **`app.js` mới** từ zip v10 (chỉ có `import './server.js';`) **hoặc** đổi Startup file thành **`server.js`**.
3. **Restart** Node app.
...
**Sửa:** Upload **`app.cjs`** từ zip v10 vào `public_html/pheduyet/`, hoặc trong Node.js App đặt **Application startup file** = `app.cjs` (file phải tồn tại).
...
| **Trang trắng `/`** | Thiếu `index.html` cùng cấp `server.js` | Không deploy chỉ `dist/` — dùng layout gói v10 |
...
./scripts/build-deploy-max-clean-v10.sh
```

Hoặc thủ công: copy các file runtime vào `deploy-max-clean-v10/` rồi `zip -r deploy-max-clean-v10.zip deploy-max-clean-v10`.
